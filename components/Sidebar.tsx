...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, isOpen }) => {
  const menuItems = [
    ...
    { id: View.DNS_RECORDS, label: 'DNS Records', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
    )},
    { id: View.DHCP_LEASES, label: 'DHCP Leases', icon: (
    ...
    { id: View.AD_BLOCKING, label: 'Ad Blocking', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
    )},
    { id: View.CONFIG, label: 'Configuration', icon: (
    ...
    { id: View.LOGS, label: 'Query Logs', icon: (
    ...
    <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      <div className="p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h2 className="font-bold text-lg leading-tight">Dnsmasq</h2>
            <p className="text-xs text-slate-500 font-medium">Admin Pro v2.4</p>
          </div>
        </div>
